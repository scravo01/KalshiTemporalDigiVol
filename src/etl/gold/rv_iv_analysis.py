...
def run() -> None:
    ...
    _print_summary(df)
    _save_data(df)
    logger.info("RV-IV analysis complete — data written to %s", RV_IV_PATH)
...
def _compute_atm_iv_5min(vol_surface_df: pl.DataFrame) -> pl.DataFrame:
    atm_per_expiry = (
        vol_surface_df
        .with_columns((pl.col("delta") - 0.5).abs().alias("_dist_atm"))
...
def _join_rv_iv(rv_df: pl.DataFrame, iv_df: pl.DataFrame) -> pl.DataFrame:
    ...
    logger.info(
        "Joined RV+IV: %d 5-min observations, %s → %s",
...
def _print_summary(df: pl.DataFrame) -> None:
    ...
    logger.info("=== RV-IV Analysis Summary ===")
...
def _save_data(df: pl.DataFrame) -> None:
    ...
    df.write_parquet(RV_IV_PATH, compression="zstd", compression_level=3)
    logger.info("Saved %d RV-IV rows → %s", len(df), RV_IV_PATH)
