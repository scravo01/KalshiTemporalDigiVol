...
with st.sidebar:
    ...
    all_dates = (
        features["trade_date"].unique().sort().to_list()
    ...
    selected_snapshots = st.multiselect(
        "Snapshots", _SNAPSHOT_ORDER, default=_SNAPSHOT_ORDER
    )
...
if not feat_filtered.is_empty() and "atm_iv" in feat_filtered.columns:
    ...
    st.line_chart(
        wide_iv.select(["trade_date"] + cols_present)
        .to_pandas()
...
if not feat_filtered.is_empty() and "skew_25d" in feat_filtered.columns:
    wide_skew = feat_filtered.pivot(
        index="trade_date", on="snapshot", values="skew_25d"
    )
    cols_present = [c for c in _SNAPSHOT_ORDER if c in wide_skew.columns]
    st.line_chart(
        wide_skew.select(["trade_date"] + cols_present)
        .to_pandas()
        .set_index("trade_date")
    )
else:
...
if not feat_filtered.is_empty():
    ...
    with col1:
        ...
        sns.boxplot(
            data=data_pd, x="snapshot", y="atm_iv", order=_SNAPSHOT_ORDER, ax=ax
        )
        ax.set_title("ATM IV")
        ax.set_xlabel("")
    ...
    with col2:
        fig, ax = plt.subplots(figsize=(5, 4))
        sns.boxplot(
            data=data_pd, x="snapshot", y="skew_25d", order=_SNAPSHOT_ORDER, ax=ax
        )
